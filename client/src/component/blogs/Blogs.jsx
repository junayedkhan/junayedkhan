...
const ACTIVE_BLOG_STORAGE_KEY = "active-blog-details";
...
const Blogs = () => {
    ...
    const activeSlug = location.pathname.startsWith("/blogs/") ? location.pathname.replace("/blogs/", "") : "";
    ...
    useEffect(() => {
        ...
        const activeBlog = blogContent.find((blog) => createBlogSlug(blog.title) === activeSlug);

        if (activeBlog) {
            setData(activeBlog);
            setopenModel(true);
            localStorage.setItem(ACTIVE_BLOG_STORAGE_KEY, activeBlog.id);
        }
    }, [activeSlug]);

    const openArticle = (article) => {
        setData(article);
        localStorage.setItem(ACTIVE_BLOG_STORAGE_KEY, article.id);
        setopenModel(true);
        navigate(`/blogs/${createBlogSlug(article.title)}`);
    };

    const closeArticle = () => {
        localStorage.removeItem(ACTIVE_BLOG_STORAGE_KEY);
        setopenModel(false);
        navigate("/blogs");
    };
    ...
    return (
        ...
        <section className="blogs">
            <div className="title_section">
                    ...
                    <h1 className="title" >travel <span>blogs</span></h1>
            </div>
        ...
        <section className={openModel ? "react_model" : "d-none"}>
            {openModel ? <div onClick={closeArticle} className="react_model overlay"></div> : null}
            <div className="react_model_inner">
                <BlogDetails setopenModel={closeArticle} data={data}/>
            </div>
        </section>
        </>
    )
